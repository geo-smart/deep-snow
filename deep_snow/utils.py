...
def create_grid(aoi, grid_size_km=100, output_shapefile='aoi_grid.shp'):
    """
    Creates a grid of bounding boxes within the specified AOI and returns a GeoDataFrame.
    Optionally saves it as a shapefile.
    ...
    Returns:
    GeoDataFrame: A GeoDataFrame containing the grid cells.
    """
    ...
    n_cols = math.ceil(width / grid_size_deg)
    n_rows = math.ceil(height / grid_size_deg)
    ...
    for i in range(n_cols):
        for j in range(n_rows):
            ...
            maxlat = min(minlat + grid_size_deg, aoi['maxlat'])
            
            grid_cells.append(box(minlon, minlat, maxlon, maxlat))

    # Create a GeoDataFrame with the grid cells
    grid_gdf = gpd.GeoDataFrame({'geometry': grid_cells}, crs='EPSG:4326')

    # Save to a shapefile if specified
    if output_shapefile:
        grid_gdf.to_file(output_shapefile)
        print(f"Shapefile created successfully: {output_shapefile}")
